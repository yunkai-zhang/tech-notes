...
class Data{
    ...
    public synchronized void increment() throws InterruptedException {
        ...
        //通知其他线程 我+1完毕了//20230223我补充：通知wait在当前data对象上的其他线程 我+1完毕了
        this.notifyAll();
    }
...
- 我理解“虚假唤醒的解决”：以两个做加法的线程为例，因为这两个线程访问“共享资源data”中的increment()，只有拿到data的锁的线程可以进入data.increment；假设线程1拿到锁，线程2就被阻塞在data.increment外：
  ...
  4. 现在两个做减法的线程公平竞争data的锁，假设某个减法线程抢到data的锁并进入data.decrement；减法线程把number--，使number回到0，便执行notifyall，通知data对象中wait的所有进程；便会通知线程1和线程2，使这两个之前释放掉锁的线程**由等待态(wait)变为阻塞态(block)下争抢锁的状态**，但是此时减法线程还没释放data的锁，所以线程1和线程2一直在准备争抢锁的状态；减法线程执行data.decrement完毕并释放data的锁，假设线程1争抢到data的锁进入就绪态等待CPU分配时间片，线程2没抢到继续处于阻塞态。
     - 问答问：等待态和阻塞态有什么区别？
       - 自答：[参考](http://t.csdn.cn/6FvVt) 所以，在我看来，BLOCKED和WAITING两个状态最大的区别有两个：
         - BLOCKED是锁竞争失败后被被动触发的状态，WAITING是人为的主动触发的状态
         - BLCKED的唤醒时自动触发的，而WAITING状态是必须要通过特定的方法来主动唤醒
     - 20230223我：线程基于obj对象wait后，会释放掉本线程拿到的obj对象的锁，并基于obj对象进入等待态；所以obj对象上所有等待态的线程被别的某线程的notifyall被唤醒后，因为都没有同步代码块的锁就都进入阻塞态（因为wait在同步代码块中），还得抢obj的锁；必须抢到锁才能解除阻塞态，线程继续执行。
...
- 这里使用CallerRunsPolicy策略，即哪来的去哪里：比如，公司让A办银行卡，银行说我这满了，你回公司让公司代你办银行卡。在本例中，是从main线程进去的（main线程让线程池执行任务），就会让main线程处理任务
...
- 我：解锁前会把内存写回主存，保证下一个线程读变量是最新的；但是这很重。使用volatile可以让所有线程都去主内存读写变量，轻量实现。（参考“什么是JMM”小节）。cas+volatile就实现了无锁并发（即乐观锁）。
...
- 思考：volatile不保证原子性，如果不加lock和synchronized ，怎么样保证原子性？
...
- 注意：原子类的包装类，底层用的是CAS；这些类的底层都直接和操作系统挂钩，在内存中修改值；
  - 20230223我：所以说乐观锁用CAS+volatile实现了无锁并发，其中CAS（比如原子类）实现了原子性，volatile实现了内存可见性。
...
- 我：[参考](http://t.csdn.cn/vEavO)，new对象不是原子性操作，包含“1.分配内存空间2.实例化对象3.给变量赋值”；指令重排可能导致先给对象赋值再实例化对象，导致单例拿到空内存，所以必须在单例引用上禁止指令重排。
