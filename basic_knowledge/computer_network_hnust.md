...
- 我：[参考](https://www.bilibili.com/read/cv5768658/)，这是网友总结的，常被分配的or保留的地址。
...
- 我：这里是ABC类网址的阶段，还没进化到划分子网的阶段，所以通过“类别”就知道哪些段决定了网络号。
- **我**：**127,128-191,192-223可以知道是ABC类网**，其实也好记，因为127是环回地址第一段，192是我们常被分配的网络的第一段，127和192之间夹着的（不包含）就是b类子网。
...
- 绿色网络的主机数量为40，加上一个路由器接口，需要可分配的IP地址数量为41，可以给该网络分配一个A类或B类或C类网络号；本着节约IP地址的原则，给该网络分配一个C类网络号。
...
- 我：使用“无分类编址”，就可以聚合之前因为地址空间太小而没有充分使用的C类网。
- 我：可能有人问，b类网在ipv4的第三段上进行划分子网，不久也相当于聚合了C类网吗？
  - 这个想法是错误的，因为b类网的ipv4第一段是[128,191]，就算在第三段上划分子网，使用的也是b类网，而没有用到ipv4第一段在[192,223]的c类网；
  - 我理解：使用无分类编码格式，就相当于无视了ipv4第一段`A[0,127] B[128,191] C[192,223]`的限制，也就用上了ipv4第一段在[192,223]的大批网址。
...
- 十分常用的**ping**命令！！！
- 我：注意ping没有通过传输层！且ping是由icmp的“询问报文”功能实现的。
...
- 我：以太网和互联网的区别，[参考](https://www.zhihu.com/question/19918493/answer/103769536)
...
- 我：路由器是划分网络的设备，默认情况下不转发广播包，ARP包只在一个局域网内有效，不需要路由器参与，由交换机来完成。[参考](https://zhidao.baidu.com/question/2265792888255295468.html)
